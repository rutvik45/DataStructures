...
int Length(struct Node *head) //function to calculate the
// length of the linked list
{
struct Node *t;
int i = 0;
if (head == NULL) //handle underflow
// condition
{
return 0;
}
...
struct Node *Start(struct Node *head, int data) //function to insert nodes at
...
struct Node *End(struct Node *head, int data) //function
// to insert nodes at the end of the list
{
...
{
a = a -> next;
} while (a -> next != head); //traverse to the
// end of the list
temp -> data = data;
temp -> next = a -> next;
a -> next = temp;
...
if (curr->next == curr) //handle
// deletion for only one node of the list
{
head = NULL;
return head;
}
...
struct Node *Position(struct Node* head, int index) //function
// to delete data from anywhere in between the start and the end of the list
{
int len = Length(head);
int count = 1;
...
while (1)
{
cout<< "Ashvin Shetty FYMCA B 104"<<endl;
